...
const updateItem = async (req, res) => {
  try {
    ...
    const item = await Items.findOne({
      where: { id: itemId, user_id: authUser },
    });
    if (!item) {
...
const getAllRentAndSale = async (req, res) => {
  ...
  try {
    ...
    const { item_type, item_category_id, city, min_budget, max_budget } =
      req.query;
...
const bookingRentItem = async (req, res) => {
  ...
  try {
    ...
    const findItem = await Items.findOne({
      where: { id: item_id, item_type: "Rent" },
      include: {
        model: Users,
        as: "user",
      },
    ...
    const bookingItem = await Rent_item_booking.create({
      item_id,
      user_id: authUser.id,
      rent_time,
      total_price: total,
    ...
    if (bookingItem) {
      const notificationData = {
        ...
        body: `${authUser.name} has requesting to book your item.`,
      };
...
const bookingSaleItem = async (req, res) => {
  ...
  try {
    ...
    const findItem = await Items.findOne({
      where: { id: item_id, item_type: "Sale" },
    });
    ...
    const bookingItem = await Sale_item_booking.create({
      item_id,
      user_id: authUser.id,
    });
