...
const updateEvent = async (req, res) => {
  try {
    ...
    const findEvent = await Event.findOne({
      where: { id: eventId, user_id: authUser },
    });
    if (!findEvent) {
    ...
    const event = await findEvent.update({
      event_title,
      event_details,
      seats,
      date,
    ...
    if (event_amenities_id) {
      ...
      const createdAmenities = await Selected_amenities.bulkCreate(
        amenitieData
      );
...
const eventBooking = async (req, res) => {
  ...
  try {
    ...
    const eventBooking = await Event_booking.create(
      { user_id: authUser.id, event_id: findEvent.id, participants },
      { transaction: trans }
    );
    if (eventBooking) {
      const notificationData = {
        title: "Event Booking Notification",
        body: `${authUser.name} has requesting to book your event.`,
      };
