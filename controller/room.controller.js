...
const addRooms = async (req, res) => {
  ...
  try {
    ...
    if (typeof req.files !== "undefined" && req.files.length > 0) {
      const isWorkspaceImg = req.files.some(
        (item) => item.fieldname === "mediaImg"
      );
      const isWorkspaceVideo = req.files.some(
        (item) => item.fieldname === "mediaVideo"
      );
      ...
      if (isWorkspaceImg || isWorkspaceVideo) {
        ...
        if (isWorkspaceImg) {
          workspace_imageArr = await FILEACTION.imageUpload(
            req.files,
            "room_image"
          );
