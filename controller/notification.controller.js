...
const notification = async (req, res) => {
  try {
    ...
    const totalCount =
      rentItemNotifications.count +
      saleItemNotifications.count +
      roomNotifications.count +
      roommateNotifications.count +
      eventNotifications.count;
...
const updateNotification = async (req, res) => {
  ...
  try {
    ...
    if (type == "Room") {
      ...
      await Rent_item_booking.update(
        { status: status },
        { where: { id: findData.id } }
      );
      ...
      await Sale_item_booking.update(
        { status: status },
        { where: { id: findData.id } }
      );
