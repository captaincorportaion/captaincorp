...
const addRoommate = async (req, res) => {
  ...
  try {
    ...
    const addRoommate = await Roommate.create({
      user_id: authUser,
      city,
      lat,
      long,
    ...
    if (addRoommate) {
      ...
      if (typeof req.files !== "undefined" && req.files.length > 0) {
        mediaUrl = await uploadRoommateFiles(
          req.files,
          "images/roommate_media"
        );
      }
...
const updateRoommate = async (req, res) => {
  try {
    ...
    const roommate = await Roommate.findOne({
      where: { id: roommateId, user_id: authUser },
    });
    if (!roommate) {
    ...
    const updateRoommate = await roommate.update({
      image: photo[0],
      city,
      lat,
      age,
    ...
    if (updateRoommate) {
      ...
      if (lifestyle_id) {
        ...
        const createdLifstyle = await SelectedLifestyle.bulkCreate(
          lifestyleData
        );
...
const bookingRoommate = async (req, res) => {
  ...
  try {
    ...
    const bookingData = await Roommate_booking.create(
      {
        date,
        minimum_stay,
        age,
        user_id: authUser.id,
        roommate_id: findRoommateData.id,
      },
      { transaction: trans }
    );
    if (bookingData) {
